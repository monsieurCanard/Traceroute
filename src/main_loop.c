...
{
	printf("%d  ", client.current_hop);
	for (int i = 0; i < MAX_ATTEMPTS; i++)
	{
		if (client.hop_packet[client.current_hop].rtt[i] < 0)
		{
			printf("*   ");
			continue;
		}

		if (i == 0 || strcmp(client.hop_packet[client.current_hop].hosts[i], client.hop_packet[client.current_hop].hosts[i - 1]) != 0	)
		{
			printf("%s (%s) ", client.hop_packet[client.current_hop].hosts[i], client.hop_packet[client.current_hop].ips[i]);
		}

		printf("%.3f ms  ",client.hop_packet[client.current_hop].rtt[i]);
	}
	printf("\n");
